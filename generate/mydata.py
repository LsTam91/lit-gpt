...
def main(
    ...
    for score_dict in rouge_scores:
        for key, value in score_dict.items():
            if key in average_rouge_scores:
                average_rouge_scores[key] += value
            else:
                average_rouge_scores[key] = value

    num_rouge_scores = len(rouge_scores)
    for key in average_rouge_scores:
        average_rouge_scores[key] /= num_rouge_scores

    # Calculate the average sacrebleu scores
    average_sacrebleu_scores = sum(score['score'] for score in sacrebleu_scores) / len(sacrebleu_scores)
